...
const nhostCompleteOrder = id => gql(`mutation($id:String!){update_orders_by_pk(pk_columns:{id:$id},_set:{status:"Completed"}){id}}`,{id:String(id)});
const nhostVoidOrder = id => gql(`mutation($id:String!){update_orders_by_pk(pk_columns:{id:$id},_set:{status:"Voided"}){id}}`,{id:String(id)});
...
async function loadFromHasura() {
  ...
  if (oData && oData.orders) {
    if (oData.orders.length > 0) {
      ...
      // Sync counter from today's orders only
      const todayPrefix = todayStr().replace(/-/g,'');
      const todayOrders = ordersData.filter(o => String(o.id).startsWith(todayPrefix));
      if(todayOrders.length > 0){
        const maxSeq = Math.max(...todayOrders.map(o => parseOrderSeq(o.id)));
        if(maxSeq >= orderCounter) orderCounter = maxSeq + 1;
      }
      document.getElementById('cart-num').textContent = 'Order ' + genOrderId();
    } else {
      ordersData = [];
      document.getElementById('cart-num').textContent = 'Order ' + genOrderId();
    }
...
// Generate date-prefixed order ID: YYYYMMDD-XXXX
function genOrderId(){
  const d = new Date();
  const prefix = d.getFullYear().toString() +
...
// Parse sequence number from a date-prefixed ID for counter sync
function parseOrderSeq(id){
  if(!id) return 0;
  const s = String(id);
  // new format: YYYYMMDD-NNNN
  const m = s.match(/-(\d+)$/);
...
let ordersData=[
  {id:new Date().toISOString().split('T')[0].replace(/-/g,'')+'-0003',customer:'Walk-in',time:'10:30 AM',item:'1x Espresso Solo (Reg)',  amount:69, status:'Completed', date:new Date().toISOString().split('T')[0]},
  {id:new Date().toISOString().split('T')[0].replace(/-/g,'')+'-0002',customer:'Walk-in',time:'10:15 AM',item:'1x Americano (Med)',      amount:94, status:'Completed', date:new Date().toISOString().split('T')[0]},
  {id:new Date().toISOString().split('T')[0].replace(/-/g,'')+'-0001',customer:'Walk-in',time:'09:50 AM',item:'1x Matcha Latte (Lrg)',  amount:144,status:'Pending',   date:new Date().toISOString().split('T')[0]},
];
...
function startOrderPolling(){
  ...
  _orderPollInterval = setInterval(async () => {
    ...
    for(const o of oData.orders){
      const existing = ordersData.find(x => String(x.id) === String(o.id));
      if(!existing){
...
function confirmOrder(cashTendered, changeAmt){
  ...
  const order={id:orderId,customer:currentUser.name,
  ...
  nhostInsertOrder({id:orderId,customer:order.customer,item:order.item,amount:order.amount,status:'Pending',time:order.time,date:todayStr()})
    .then(res => {
...
function completeOrder(id){
  const o = ordersData.find(o => String(o.id) === String(id));
  if(!o) return;
  o.status = 'Completed';
  renderOrders(); renderMiniOrders(); updatePendingBadge();
  toast('Order ' + id + ' completed ✓');
  nhostCompleteOrder(id); // push to Hasura async — poll won't downgrade since STATUS_RANK guards it
}
