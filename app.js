...
async function clearTodayLogs(){
  ...
  if(!confirm(`Delete ALL attendance logs for ${today}? This cannot be undone.`)) return;
  
  const todayLogs = attData.filter(a => a.date === today);
  if(todayLogs.length === 0){ showToast('No logs for today.'); return; }

  for(const log of todayLogs){
    await gql(`mutation($id:Int!){delete_attendance_by_pk(id:$id){id}}`, {id: log.id});
  }

  attData = attData.filter(a => a.date !== today);
  renderAtt();
  showToast('Today\'s logs cleared!');
}
