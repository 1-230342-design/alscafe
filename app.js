...
const nhostUpdateStock = (id,stock) => gql(`mutation($id:String!,$s:numeric!){update_inventory_by_pk(pk_columns:{id:$id},_set:{stock:$s}){id}}`,{id,s:parseFloat(parseFloat(stock).toFixed(4))});
...
const nhostCompleteOrder = id => gql(`mutation($id:Int!){update_orders_by_pk(pk_columns:{id:$id},_set:{status:"Completed"}){id}}`,{id:parseInt(id)});
const nhostVoidOrder    = id => gql(`mutation($id:Int!){update_orders_by_pk(pk_columns:{id:$id},_set:{status:"Voided"}){id}}`,{id:parseInt(id)});
...
async function loadFromHasura() {
  // Pull inventory from DB, then sync: update local stock from DB for known IDs,
  // purge rogue DB rows, and upsert any local items missing from DB.
  ...
  // Build flat list of all canonical local items
  const allLocalItems = Object.entries(invData).flatMap(([cat, items]) =>
    items.map(i => ({...i, category: cat}))
  );
  const validIds = new Set(allLocalItems.map(i => i.id));

  if (iData && iData.inventory) {
    const dbIds = new Set(iData.inventory.map(r => r.id));

    // 1. Update stock for IDs that exist in both local and DB
    for(const dbItem of iData.inventory){
      if(validIds.has(dbItem.id)){
        for(const cat of Object.keys(invData)){
          const local = invData[cat].find(i => i.id === dbItem.id);
          if(local){
            local.stock = dbItem.stock;
            local.threshold = dbItem.threshold;
            if(dbItem.last_restocked) local.lastRestocked = dbItem.last_restocked;
          }
        }
      } else {
        // 2. Purge rogue DB rows that aren't in our local canonical list
        console.warn('[Hasura] Purging rogue inventory row:', dbItem.id, dbItem.name);
        gqlSilent('mutation($id:String!){delete_inventory_by_pk(id:$id){id}}', {id: dbItem.id});
      }
    }

    // 3. Upsert any local items missing from DB so DB stays complete
    for(const local of allLocalItems){
      if(!dbIds.has(local.id)){
        console.log('[Hasura] Upserting missing item to DB:', local.id, local.name);
        gqlSilent(`mutation($obj:inventory_insert_input!){
          insert_inventory_one(object:$obj,
            on_conflict:{constraint:inventory_pkey, update_columns:[name,category,unit,stock,threshold,supplier,last_restocked,notes]}
          ){id}}`,
          {obj:{id:local.id, name:local.name, category:local.category, unit:local.unit,
                stock:local.stock, threshold:local.threshold, supplier:local.supplier,
                last_restocked:local.lastRestocked, notes:local.notes||''}});
      }
    }
  }
  renderInv(); checkLowStock(); renderInvCats();
  // Load orders from Hasura
  const oData = await gql('{ orders(order_by:{id:desc}) { id customer item amount status time date } }');
  ...
  if (oData && oData.orders) {
    if (oData.orders.length > 0) {
      ...
      const maxId = Math.max(...ordersData.map(o => parseInt(o.id)||0));
...
let invData = {
  'Espresso':[
    {id:'INV-031',name:'Arabica Beans',        unit:'Grams (g)',    stock:200, threshold:50, lastRestocked:'2026-03-01',supplier:'CoffeeHub PH',    notes:'Premium Grade A — used in all espresso drinks'},
  ...
  'Iced':[
    ...
    {id:'INV-043',name:'Filtered Water', unit:'Liters (L)',     stock:200, threshold:0, lastRestocked:'2026-03-10',supplier:'IcePack MNL',notes:'Base for iced drinks'},
  ],
  'Pastries':[
...
// MENU DATA
// ══════════════════════════════════════════════
let currentUser={}, cart=[], orderCounter=1, dineMode='dine-in';
...
let ordersData=[
  {id:3,customer:'Walk-in',time:'10:30 AM',item:'1x Espresso Solo (Reg)',  amount:69, status:'Completed', date:new Date().toISOString().split('T')[0]},
  {id:2,customer:'Walk-in',time:'10:15 AM',item:'1x Americano (Med)',      amount:94, status:'Completed', date:new Date().toISOString().split('T')[0]},
  {id:1,customer:'Walk-in',time:'09:50 AM',item:'1x Matcha Latte (Lrg)',  amount:144,status:'Pending',   date:new Date().toISOString().split('T')[0]},
];
...
// Liquid amounts (milk, water, syrup) stored in mL — deductInventory
// divides by 1000 before subtracting from the Liters(L) inventory.
// Solids (beans g, powder g, ice kg, cups/straws pcs) stored as-is.
//
// Cup sizes: Regular=240mL | Medium=350mL | Large=470mL
// SIZE_SCALE applies to liquid & solid ingredients; cups/straws always qty 1.
//
...
// ─────────────────────────────────────────────────────────────────
const RECIPES = {
  // ── ESPRESSO (hot) — Regular = 240 mL cup ────────────────────
  // Beans(g) | Water(mL) | Milk(mL) | Hot Cup
  'Espresso Solo': [['INV-031',18], ['INV-043',60],  ['HOT_CUP',1]],
  'Americano':     [['INV-031',18], ['INV-043',150], ['HOT_CUP',1]],
  'Cappuccino':    [['INV-031',18], ['INV-043',50],  ['INV-042',120], ['HOT_CUP',1]],
  'Macchiato':     [['INV-031',18], ['INV-043',40],  ['INV-042',50],  ['HOT_CUP',1]],
  'Lungo':         [['INV-031',22], ['INV-043',180], ['HOT_CUP',1]],
  'Ristretto':     [['INV-031',14], ['INV-043',40],  ['HOT_CUP',1]],

  // ── MILK-BASED (hot) ─────────────────────────────────────────
  'Latte':      [['INV-031',18], ['INV-043',50],  ['INV-042',160], ['INV-044',10], ['HOT_CUP',1]],
  'Flat White': [['INV-031',18], ['INV-043',40],  ['INV-042',150], ['HOT_CUP',1]],
  'Mocha':      [['INV-031',18], ['INV-043',50],  ['INV-042',150], ['INV-010',20], ['HOT_CUP',1]],

  // ── NON-COFFEE (hot) ─────────────────────────────────────────
  'Matcha Latte':  [['INV-008',8],  ['INV-043',50],  ['INV-042',160], ['INV-044',10], ['HOT_CUP',1]],
  'Chamomile Tea': [['INV-009',1],  ['INV-043',200], ['INV-044',10],  ['HOT_CUP',1]],
  'Hot Choco':     [['INV-010',20], ['INV-043',50],  ['INV-042',150], ['INV-044',10], ['HOT_CUP',1]],

  // ── ICED — Regular = 240 mL usable volume ────────────────────
  'Iced Americano': [['INV-031',18], ['INV-043',100], ['INV-011',0.2],  ['INV-044',10], ['ICED_CUP',1], ['ICED_STRAW',1]],
  'Iced Latte':     [['INV-031',18], ['INV-043',50],  ['INV-042',150], ['INV-011',0.2], ['INV-044',10], ['ICED_CUP',1], ['ICED_STRAW',1]],
  'Iced Matcha':    [['INV-008',10], ['INV-043',50],  ['INV-042',160], ['INV-011',0.2], ['INV-044',10], ['ICED_CUP',1], ['ICED_STRAW',1]],
...
// Scale ingredient deductions by size
// Regular=240mL, Medium=350mL (~1.46×), Large=470mL (~1.96×)
const SIZE_SCALE = {'Regular':1.0,'Medium':1.46,'Large':1.96,'':1.0};

// IDs whose recipe amounts are in mL (inventory stored in Liters → divide by 1000 to deduct)
const ML_INV_IDS = new Set(['INV-042','INV-041','INV-044','INV-043','INV-007']);
...
function deductInventory(cartItems){
  ...
  for(const ci of cartItems){
    ...
    // Track which inventory IDs have already been deducted for THIS cart item
    // (same ID may appear in multiple categories — only deduct once)
    const deductedIds = new Set();

    for(let [invId, baseAmt] of recipe){
      const isCupOrStraw = NO_SCALE_SENTINELS.has(invId);
      // Cups/straws: always 1 per drink, never size-scaled
      // mL liquid ingredients: scale by size, then multiply by qty
      const scaledAmt = isCupOrStraw
        ? baseAmt * ci.qty
        : baseAmt * sizeScale * ci.qty;

      // Resolve cup/straw sentinels
      ...
      for(const cat of Object.keys(invData)){
        ...
        if(!item) continue;
        // Skip if we've already deducted this inventory ID for this cart item
        if(deductedIds.has(invId)) continue;
        deductedIds.add(invId);

        const isLiquid = ML_INV_IDS.has(invId);
        // Amount to actually deduct from inventory (Liters if liquid, as-is otherwise)
        // Use parseFloat (not Math.round) for non-liquids so fractional kg (ice) aren't zeroed out
        const deductAmt = isLiquid
          ? parseFloat((scaledAmt / 1000).toFixed(6))  // mL → L
          : parseFloat(scaledAmt.toFixed(4));
        // Display label for the receipt
        const displayLabel = isLiquid
          ? `${Math.round(scaledAmt)} mL`
          : `${Math.round(scaledAmt)} ${item.unit}`;

        const prevStock = item.stock;
        item.stock = parseFloat(Math.max(0, item.stock - deductAmt).toFixed(4));
        log.push(`${item.name} −${displayLabel}`);

        nhostUpdateStock(item.id, item.stock);
        nhostInsertInvLog({
          inventory_id: item.id, item_name: item.name,
          qty_deducted: deductAmt, unit: item.unit,
        ...
        if(item.threshold > 0 && item.stock < item.threshold){
...
function doLogin(){
  ...
  checkNhost();
  // ── Log login event to Hasura ──
  gqlSilent(
    `mutation($obj:login_logs_insert_input!){insert_login_logs_one(object:$obj){id}}`,
    {obj:{emp_id:user.empId, name:user.name, role:user.role, access:user.access, logged_in_at:new Date().toISOString()}}
  ).then(res => {
    if(!res) {
      // Fallback: try upserting into the users table instead (if login_logs doesn't exist)
      gqlSilent(
        `mutation($obj:users_insert_input!){insert_users_one(object:$obj,on_conflict:{constraint:users_pkey,update_columns:[last_login,role,access]}){emp_id}}`,
        {obj:{emp_id:user.empId, name:user.name, role:user.role, access:user.access, last_login:new Date().toISOString()}}
      );
    }
  });
  loadFromHasura().then(() => {
    // ── Auto Time-In on login for Employee accounts ──
    // After DB loads, check if this employee already has a record today.
    // If not, automatically insert a Time In attendance record to Hasura.
    if(currentUser.access === 'Employee Access'){
      ...
      if(!existing){
        ...
        gql(
          ...
          {obj:{emp_id:record.empId, name:record.name, date:record.date,
                time_in:record.timeIn, time_out:'--:--', hours:'--',
...
function doLogout(){
  ...
  if(_attPollInterval){   clearInterval(_attPollInterval);   _attPollInterval   = null; }
...
function initApp(){
  ...
  if(isAdmin){
   ...
   const ordersDateEl = document.getElementById('orders-date-filter');
  if(ordersDateEl) ordersDateEl.value = todayStr();
  ...
  // ── Live polling: auto-refresh data from Hasura ──
  // Orders: every 3s | Inventory: every 5s | Attendance: every 5s
  startOrderPolling();
...
let _attPollInterval   = null;
// Status priority — higher number = more final; poll only upgrades, never downgrades
const STATUS_RANK = {'Pending':0,'Completed':1,'Voided':2};
function startOrderPolling(){
  if(_orderPollInterval) clearInterval(_orderPollInterval);
  _orderPollInterval = setInterval(async () => {
    const oData = await gqlSilent('{ orders(order_by:{id:desc},limit:100) { id customer item amount status time date } }');
    if(!oData || !oData.orders) return;
    let changed = false;
    for(const o of oData.orders){
      const existing = ordersData.find(x => x.id === o.id || String(x.id) === String(o.id));
      if(!existing){
        ordersData.unshift({id:o.id, customer:o.customer, time:o.time||'--',
          item:o.item, amount:o.amount, status:o.status, date:o.date||todayStr()});
        changed = true;
      } else {
        const localRank = STATUS_RANK[existing.status] ?? 0;
        const dbRank    = STATUS_RANK[o.status] ?? 0;
        if(dbRank > localRank){ existing.status = o.status; changed = true; }
      }
    }
    if(changed){ renderOrders(); renderMiniOrders(); updatePendingBadge(); }
  }, 3000); // reduced from 8s to 3s for near-instant updates
}

function startInventoryPolling(){
  if(_invPollInterval) clearInterval(_invPollInterval);
  _invPollInterval = setInterval(async () => {
    const iData = await gqlSilent('{ inventory(order_by:{id:asc}) { id stock threshold } }');
    if(!iData || !iData.inventory) return;
    let changed = false;
    for(const dbItem of iData.inventory){
      for(const cat of Object.keys(invData)){
        const local = invData[cat].find(i => i.id === dbItem.id);
        if(local && (local.stock !== dbItem.stock || local.threshold !== dbItem.threshold)){
          local.stock = dbItem.stock;
          local.threshold = dbItem.threshold;
          changed = true;
        }
    ...
    if(changed){ renderInv(); checkLowStock(); renderInvCats(); }
  }, 5000); // inventory refreshes every 5s
}

function startAttendancePolling(){
  if(_attPollInterval) clearInterval(_attPollInterval);
  _attPollInterval = setInterval(async () => {
    const aData = await gqlSilent('{ attendance(order_by:{id:desc}) { id emp_id name date time_in time_out hours status notes } }');
    if(!aData || !aData.attendance) return;
    const fresh = aData.attendance.map(a => ({
      id:a.id, empId:a.emp_id, name:a.name, date:a.date,
      timeIn:a.time_in, timeOut:a.time_out, hours:a.hours,
      status:a.status, notes:a.notes||''
    }));
    if(JSON.stringify(fresh) !== JSON.stringify(attData)){
      attData = fresh;
      fillAttDropdown(); renderAtt(); syncTcState();
    }
  }, 5000); // attendance refreshes every 5s
}
...
// ══════════════════════════════════════════════
function renderCats(){
  document.getElementById('cats-row').innerHTML=MENU_CATS.map(c=>`
...
function addToCart(itemId,size){
  ...
  if(recipe){
    ...
    for(let [invId,qty] of recipe){
      ...
      if(!inv) continue;
      // Liquid ingredients stored in mL in recipes but Liters in inventory — convert before comparing
      const isLiquid = ML_INV_IDS.has(invId);
      const compareQty = isLiquid ? scaledQty / 1000 : scaledQty;
      if(inv.stock < compareQty){
        const needDisplay = isLiquid ? parseFloat((scaledQty/1000).toFixed(4)) : scaledQty;
        insufficient.push({name:inv.name,have:inv.stock,need:needDisplay,unit:inv.unit,status:inv.stock<=0?'OUT':'LOW'});
      }
    }
...
function confirmOrder(cashTendered, changeAmt){
  ...
  // Use a temporary local ID; will be replaced with DB-assigned int after insert
  const tempId = orderCounter;
  const order={id:tempId, customer:currentUser.name,
  ...
  // Let Hasura auto-assign the integer ID
  nhostInsertOrder({customer:order.customer, item:order.item, amount:order.amount, status:'Pending', time:order.time, date:todayStr()})
    .then(res => {
      if(res && res.insert_orders_one){
        const realId = res.insert_orders_one.id;
        order.id = realId;
        if(realId >= orderCounter) orderCounter = realId + 1;
        document.getElementById('cart-num').textContent = 'Order #' + orderCounter;
        renderOrders(); renderMiniOrders();
      }
    });
...
function completeOrder(id){
  const o = ordersData.find(o => o.id === id || String(o.id) === String(id));
  if(!o) return;
  o.status = 'Completed';
  renderOrders(); renderMiniOrders(); updatePendingBadge();
  toast('Order #' + id + ' completed ✓');
  nhostCompleteOrder(id);
}
...
function renderMiniOrders(){
  const today = todayStr();
  const todayOrders = ordersData.filter(o => (o.date || today) === today);
  document.getElementById('mini-orders-body').innerHTML=todayOrders.slice(0,5).map(o=>`
    <tr>
      ...
      <td style="text-align:right;font-weight:700">₱${Number(o.amount).toLocaleString('en-PH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td>
...
function renderOrders(){
  ...
  const filterDate = (dateEl && dateEl.value) ? dateEl.value : todayStr();
  const filtered = ordersData.filter(o => (o.date || todayStr()) === filterDate);

  const total = filtered.filter(o=>o.status!=='Voided').reduce((s,o)=>s+o.amount,0);
  ...
  const makeRow = o => `
    <tr style="${o.status==='Voided'?'opacity:0.5;text-decoration:line-through':''}">
      <td class="otbl-td"><strong>${o.customer}</strong><br><span style="font-size:11px;color:var(--text-light)">${o.time}</span></td>
      <td class="otbl-td">#${o.id}</td>
      <td class="otbl-td" style="font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${o.item}</td>
      <td class="otbl-td otbl-amt">₱${Number(o.amount).toLocaleString('en-PH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td>
      <td class="otbl-td" style="text-decoration:none">${statusBadge(o)}</td>
      <td class="otbl-td" style="text-decoration:none">${actionCell(o)}</td>
    </tr>`;

  const makeTable = (rows, emptyMsg) => `
    <div class="order-card" style="overflow-x:auto">
      <table style="width:100%;border-collapse:collapse;font-size:13px;min-width:560px;table-layout:fixed">
        <colgroup>
          <col style="width:18%">
          <col style="width:14%">
          <col style="width:28%">
        ...
        <thead>
          <tr style="background:var(--green-bg)">
            <th class="otbl-th">Employee</th>
            <th class="otbl-th">Order #</th>
            <th class="otbl-th">Items</th>
            <th class="otbl-th otbl-amt">Amount</th>
            <th class="otbl-th">Status</th>
            <th class="otbl-th">Actions</th>
          </tr>
        </thead>
        <tbody>
          ${rows.length ? rows.join('') : `<tr><td colspan="6" style="padding:16px;color:var(--text-light);font-size:13px;text-align:center">${emptyMsg}</td></tr>`}
        </tbody>
      </table>
    </div>`;
  ...
  if(pc) pc.innerHTML = makeTable(pending.map(makeRow), 'No pending orders.');

  const cc = document.getElementById('completed-container');
  if(cc){ cc.innerHTML = makeTable(completed.map(makeRow), 'No completed orders yet.'); cc.closest('.orders-sec').style.display=''; }

  const vs = document.getElementById('voided-orders-sec');
  const vc = document.getElementById('voided-container');
  if(vs) vs.style.display = voided.length ? 'block' : 'none';
  if(vc) vc.innerHTML = makeTable(voided.map(makeRow), 'No voided orders.');

  updatePendingBadge();
}
function clearVoided(id){
  if(id !== undefined) ordersData=ordersData.filter(o=>o.id!==id);
...
function addOrderManual(){
  ...
  const manualOrder = {id:orderCounter++,customer:cust,time,item:itemStr,amount:amt,status:'Pending',date:todayStr()};
  ordersData.unshift(manualOrder);
  nhostInsertOrder({customer:cust,item:itemStr,amount:amt,status:'Pending',time,date:todayStr()})
    .then(res => {
      if(res && res.insert_orders_one) {
        manualOrder.id = res.insert_orders_one.id;
        renderOrders(); renderMiniOrders();
      }
    });
...
async function doTimeIn(){
  ...
  const now = new Date();
  const timeInStr = fmtTime(now);
  const status = calcStatus(now, null);
  // Insert to Hasura FIRST — get the real DB-assigned ID before updating local state
  toast('⏳ Recording Time In...');
  const res = await gql(
    `mutation($obj:attendance_insert_input!){insert_attendance_one(object:$obj){id}}`,
    {obj:{emp_id:currentUser.empId, name:currentUser.name, date:today,
          time_in:timeInStr, time_out:'--:--', hours:'--', status:status, notes:''}}
  );
  if(!res || !res.insert_attendance_one){
    toast('❌ Time In failed — check DB connection'); return;
  }
  const record = {
    id: res.insert_attendance_one.id,  // real Hasura ID
    empId: currentUser.empId,
    name: currentUser.name,
    date: today,
    timeIn: timeInStr,
...
async function doTimeOut(){
  ...
  // Guard: if record has no valid DB id, re-fetch from Hasura to get it
  if(!record.id || isNaN(parseInt(record.id))){
    toast('⏳ Syncing record...');
    const check = await gql(`{ attendance(where:{emp_id:{_eq:"${currentUser.empId}"},date:{_eq:"${today}"}},limit:1){ id } }`);
    if(check && check.attendance && check.attendance[0]){
      record.id = check.attendance[0].id;
    } else {
      toast('❌ No Time In record found in DB — please Time In first'); return;
    }
  }
  const now = new Date();
  ...
  const res = await gql(
    ...
    {id:parseInt(record.id), to:timeOutStr, hrs:hrs, st:status}
  );
  if(!res || !res.update_attendance_by_pk){
    toast('❌ Time Out failed — check DB connection'); return;
  }
