...
let ordersData=[
  {id:3300,customer:'Walk-in',time:'10:30 AM',item:'1x Espresso Solo (Reg)',  amount:69, status:'Completed', date:new Date().toISOString().split('T')[0]},
  {id:3299,customer:'Walk-in',time:'10:15 AM',item:'1x Americano (Med)',      amount:94, status:'Completed', date:new Date().toISOString().split('T')[0]},
  {id:3298,customer:'Walk-in',time:'09:50 AM',item:'1x Matcha Latte (Lrg)',  amount:144,status:'Pending',   date:new Date().toISOString().split('T')[0]},
];
  ...
  nhostInsertOrder({customer:order.customer,item:order.item,amount:order.amount,status:'Pending',time:order.time,date:todayStr()})
    .then(res => {
      if(res && res.insert_orders_one) {
        order.id = res.insert_orders_one.id;
        renderOrders(); renderMiniOrders();
      }
    });
