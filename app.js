...
async function loadFromHasura() {
  ...
  if (iData && iData.inventory && iData.inventory.length > 0) {
    // SAFE MERGE — update stock/threshold from DB without wiping local structure.
    // Prevents AI-prompt-corrupted or partial DB data from destroying invData.
    const dbMap = {};
    for(const i of iData.inventory) dbMap[i.id] = i;
    // Update matching items in place; first occurrence wins for duplicate IDs across cats
    const seenIds = new Set();
    for(const cat of Object.keys(invData)){
      for(const item of invData[cat]){
        if(seenIds.has(item.id)) continue;
        seenIds.add(item.id);
        if(dbMap[item.id]){
          const db = dbMap[item.id];
          item.stock          = db.stock;
          item.threshold      = db.threshold;
          if(db.last_restocked) item.lastRestocked = db.last_restocked;
          if(db.supplier)       item.supplier      = db.supplier;
          if(db.notes)          item.notes         = db.notes;
        }
      }
    }
    // Add any genuinely new items from DB that don't exist locally
    for(const i of iData.inventory){
      if(!seenIds.has(i.id)){
        const cat = i.category;
        if(!invData[cat]) invData[cat] = [];
        if(!invData[cat].find(x => x.id === i.id)){
          invData[cat].push({
...
function deductInventory(cartItems){
  ...
  for(const ci of cartItems){
    ...
    for(let [invId, baseAmt] of recipe){
      ...
      for(const cat of Object.keys(invData)){
        ...
        // Only auto-remove when a real threshold is set AND stock has fallen below it.
        // threshold:0 means "no threshold" — never auto-remove those items.
  ...
  // Deduplicate removed list by id, then purge from ALL categories
  const removedMap = {};
  for(const r of removed) if(!removedMap[r.id]) removedMap[r.id] = r;
  for(const r of Object.values(removedMap)){
    for(const cat of Object.keys(invData)){
      invData[cat] = invData[cat].filter(i => i.id !== r.id);
    }
