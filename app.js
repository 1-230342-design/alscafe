...
function startOrderPolling(){
  ...
  _orderPollInterval = setInterval(async () => {
    const oData = await gqlSilent('{ orders(order_by:{id:desc}) { id customer item amount status time date } }');
    ...
    for(const o of oData.orders){
      ...
      if(!existing){
        ...
        changed = true;
      } else if(existing.status !== o.status){
        // Status changed (e.g. completed from another terminal)
...
function renderOrders(){
  ...
  const makeRow = o => `
    <tr style="${o.status==='Voided'?'opacity:0.5;text-decoration:line-through':''}">
      ...
      <td style="text-align:right;font-weight:700">\u20b1${o.amount.toLocaleString('en-PH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px;min-width:520px">
        <thead>
          <tr style="background:var(--green-bg)">
            ...
            <th style="padding:11px 13px;text-align:right;font-size:11px;font-weight:700;color:var(--text-mid);text-transform:uppercase;letter-spacing:0.6px">Amount</th>
