...
async function loadFromHasura() {
  // Load inventory from Hasura — ONLY update stock/threshold for IDs already
  // defined locally. Never add, rename, or restructure categories from DB data.
  // This prevents AI-prompt-corrupted DB rows from polluting the local inventory.
  ...
  if (iData && iData.inventory && iData.inventory.length > 0) {
    // Build a set of all valid IDs that exist in local invData
    const validIds = new Set(Object.values(invData).flat().map(i => i.id));

    // For every DB row: if the ID is in our local structure, update stock only.
    // If the ID is NOT in our local structure, it's a rogue/corrupted row — delete it from DB.
    for(const dbItem of iData.inventory){
      if(validIds.has(dbItem.id)){
        // Update stock & threshold in every category this ID appears in
        ...
        // Rogue row not in our local structure — purge it from Hasura silently
        console.warn('[Hasura] Purging unknown inventory row:', dbItem.id, dbItem.name);
...
let invData = {
  // ── CANONICAL ITEM LIST — this is the single source of truth ──
  // Only these IDs are valid. Any other ID in the DB will be auto-purged on login.
  //
  // INV-031 = Arabica Beans        INV-002 = Espresso Roast Beans
  // INV-044 = Sugar Syrup          INV-042 = Whole Milk
  // INV-041 = Oat Milk             INV-007 = Skimmed Milk
  // INV-009 = Chamomile Bags       INV-008 = Matcha Powder
