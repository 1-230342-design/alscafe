...
async function loadFromHasura() {
  ...
  if (iData && iData.inventory && iData.inventory.length > 0) {
    ...
    for(const i of iData.inventory){
      // Normalize category casing to match CATS names exactly
      const rawCat = i.category || '';
      const cat = CATS.find(c => c.name.toLowerCase() === rawCat.toLowerCase())?.name || rawCat;
    ...
    // Merge Hasura data with local defaults — items missing from DB stay available locally
    // This prevents silently skipping ingredients (e.g. straws) that exist locally but not in DB
    for(const cat of Object.keys(invData)){
      const dbItems = freshInv[cat] || [];
      const dbIds = new Set(dbItems.map(i => i.id));
      const localItems = invData[cat] || [];
      // Keep local items that aren't in DB yet
      const localOnly = localItems.filter(i => !dbIds.has(i.id));
      freshInv[cat] = [...dbItems, ...localOnly];
    }
    invData = freshInv;
    console.log('[Hasura] merged invData keys:', Object.keys(invData), '| total items:', Object.values(invData).flat().length);
...
// Canonical name map: recipe ID -> lowercase item name
// Used as fallback when Hasura DB uses different IDs than the recipe constants
// ID_NAME_MAP: recipe constant ID -> exact DB item name (lowercase)
// Only used as fallback when direct ID lookup fails
// DB names taken from Hasura screenshots — must match exactly
const ID_NAME_MAP = {
  'INV-031':'arabica beans',
  'INV-002':'espresso roast beans',
  'INV-042':'whole milk',       // Milk-based category
  'INV-041':'oat milk',         // Milk-based category
  'INV-044':'sugar syrup',
  'INV-043':'filtered water',
  'INV-007':'skimmed milk',
  'INV-008':'matcha powder',
  'INV-009':'chamomile bags',
  'INV-010':'cocoa powder',
  'INV-011':'ice cubes',
  'INV-014':'croissants',
  'INV-015':'muffin mix',
  'INV-016':'chocolate chips',
  'INV-034':'hot cups – regular (8oz)',
  'INV-035':'hot cups – medium (12oz)',
  'INV-036':'hot cups – large (16oz)',
  'INV-037':'iced cups – regular (16oz)',
  'INV-038':'iced cups – medium (22oz)',
  'INV-039':'iced cups – large (24oz)',
  'INV-040':'straws'
};
...
const ML_INV_IDS = new Set(['INV-042','INV-041','INV-044','INV-043','INV-007','INV-003','INV-004']);
// isLiquidItem: true for any item whose unit is Liters — catches DB items with unexpected IDs
function isLiquidItem(item){ return item && (item.unit==='Liters (L)' || item.unit==='L'); }
...
function deductInventory(cartItems){
  ...
  // Flat list of all inventory items across all categories for reliable lookup
  const allItems = Object.values(invData).flat();
  // Build name-based fallback lookup (normalized lowercase) for when DB IDs differ from recipe IDs
  const invByName = {};
  for(const it of allItems) invByName[it.name.toLowerCase().trim()] = it;
  // ID remap: if recipe ID not found, try to match by canonical name
  // ID_NAME_MAP is global — defined near ICED_STRAW_ID
  function findInvItem(id){
    // Try direct ID match first (most reliable)
    let it = allItems.find(i => i.id === id);
    if(it) return it;
    // Fall back: match by canonical name, but prefer item whose ID matches closest
    const name = ID_NAME_MAP[id];
    if(!name) return null;
    // Find all items with this name, prefer the one with the exact expected ID
    const matches = allItems.filter(i => i.name.toLowerCase().trim() === name);
    if(!matches.length) return null;
    // Prefer exact ID match if somehow missed above, else first match
    return matches.find(i => i.id === id) || matches[0];
  }
  console.log('[deductInventory] allItems IDs:', allItems.map(i=>i.id));
  ...
  for(const ci of cartItems){
    ...
    for(let [invId, baseAmt] of recipe){
      ...
      // Find item by ID, with name-based fallback for DB ID mismatches
      const item = findInvItem(invId);
      if(!item){ console.warn('[deductInventory] NOT FOUND in invData:', invId); continue; }
      if(deductedIds.has(invId)) continue;
      deductedIds.add(invId);
      {
        const cat = Object.keys(invData).find(c => invData[c].includes(item)) || '';
...
function initApp(){
  ...
  // ── Live polling: auto-refresh orders from Hasura every 8 seconds ──
  // This means completing an order or receiving a new order updates instantly without a page refresh.
  startOrderPolling();
}
...
function startOrderPolling(){
  ...
  _orderPollInterval = setInterval(async () => {
    ...
    for(const o of oData.orders){
      ...
      if(!existing){
        // New order from another terminal — add it
        ...
        // Only update status if DB has a MORE final status (never revert Completed→Pending)
...
function addToCart(itemId,size){
  ...
  if(recipe){
    ...
    const allInvByName={};
    for(const it of allInv) allInvByName[it.name.toLowerCase().trim()]=it;
    const insufficient=[];
    ...
    for(let [invId,qty] of recipe){
      ...
      if(invId==='ICED_STRAW')invId=ICED_STRAW_ID;
      let inv=allInv.find(i=>i.id===invId);
      if(!inv && ID_NAME_MAP[invId]) inv=allInvByName[ID_NAME_MAP[invId]];
      if(!inv) continue;
      // Liquid ingredients stored in mL in recipes but Liters in inventory — convert before comparing
      const isLiquid = isLiquidItem(inv);
...
async function completeOrder(id){
  ...
  // Push to Hasura and confirm
  const res = await nhostCompleteOrder(id);
  if(res && res.update_orders_by_pk){
    toast('\u2705 Order #' + id + ' saved to Hasura');
  } else {
    // Hasura failed — revert so local stays in sync with DB
    o.status = 'Pending';
    renderOrders(); renderMiniOrders(); updatePendingBadge();
    toast('\u26a0\ufe0f Could not update Order #' + id + ' in Hasura — check connection');
  }
}
...
function renderInv(){
  ...
  document.getElementById('inv-tbl-body').innerHTML=items.map(i=>{
    ...
    return `<tr style="${bg}">${cb}
      ...
      <td>${i.unit}</td><td><strong>${parseFloat(i.stock).toFixed(2)}</strong></td><td>${i.threshold > 0 ? i.threshold : '—'}</td><td>${s}</td>
...
function saveInvEdit(id){
  ...
  const newStock=parseFloat(document.getElementById('ei-stock').value)||0;
  item.stock=newStock;
